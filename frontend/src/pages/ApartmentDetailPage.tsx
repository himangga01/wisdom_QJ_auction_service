...
export function ApartmentDetailPage() {
  ...
  const requestedCompareRunId = searchParams.get('compareRunId')
  const focusListingId = searchParams.get('focusListingId') ?? undefined
  const apartmentQuery = useQuery({
    ...
    enabled: !analysis.isDemo && Boolean(complexId && selectedSourceId),
  })
  ...
  const apiHistory = apartmentQuery.data?.history ?? []
  const requestedRunMissing = Boolean(
    apartmentQuery.data
    && requestedRunId
    && !apiHistory.some((point) => point.runId === requestedRunId),
  )
  const requestedCompareRunMissing = Boolean(
    apartmentQuery.data
    && requestedCompareRunId
    && !apiHistory.some((point) => point.runId === requestedCompareRunId),
  )

  useEffect(() => {
    const history = apartmentQuery.data?.history ?? []
    if (requestedRunMissing || requestedCompareRunMissing) {
      setSelectedDate('')
      setComparisonDate('')
      return
    }
    const requestedDate = history.find((point) => point.runId === requestedRunId)?.collectedAt
    const selected = requestedDate ?? dates.at(-1) ?? ''
    const requestedComparisonDate = history.find((point) => point.runId === requestedCompareRunId)?.collectedAt
    const selectedIndex = dates.indexOf(selected)
    setSelectedDate(selected)
    setComparisonDate(requestedComparisonDate ?? (selectedIndex > 0 ? dates[selectedIndex - 1] : ''))
    setTradeFilter('all')
    setListingViewMode('card')
  }, [
    apartmentQuery.data?.history,
    complexId,
    dates,
    requestedCompareRunId,
    requestedCompareRunMissing,
    requestedRunId,
    requestedRunMissing,
  ])
  ...
  useEffect(() => {
    ...
    if (
      ...
      || !apartment
      || (
        selectedApartment?.sourceId === apartment.sourceId
        && selectedApartment.complexId === apartment.complexId
      )
    ) return
  ...
  const selectedApartmentQuery = useQuery({
    queryKey: apartmentKeys.detail(complexId ?? '', selectedRunId, selectedSourceId),
    queryFn: () => getApartment(
      complexId as string,
      selectedSourceId as string,
      selectedRunId,
    ),
    enabled: !analysis.isDemo && !requestedRunMissing && !requestedCompareRunMissing && Boolean(complexId && selectedSourceId && selectedRunId),
  })
  const selectedListingsQuery = useQuery({
    queryKey: apartmentKeys.listings(complexId ?? '', selectedRunId, selectedSourceId),
    queryFn: () => getApartmentListings(complexId as string, { sourceId: selectedSourceId as string, runId: selectedRunId }),
    enabled: !analysis.isDemo && !requestedRunMissing && !requestedCompareRunMissing && Boolean(complexId && selectedSourceId && selectedRunId),
  })
  const comparisonListingsQuery = useQuery({
    queryKey: apartmentKeys.listings(complexId ?? '', comparisonRunId, selectedSourceId),
    queryFn: () => getApartmentListings(complexId as string, { sourceId: selectedSourceId as string, runId: comparisonRunId }),
    enabled: !analysis.isDemo && !requestedRunMissing && !requestedCompareRunMissing && Boolean(complexId && selectedSourceId && comparisonRunId),
  })
  ...
  const selectedDetailQueries = useQueries({
    queries: selectedListingRecords.map((record) => ({
      queryKey: apartmentKeys.listing(record.groupId, selectedRunId, selectedSourceId),
      queryFn: () => getListing(record.groupId, selectedSourceId as string, selectedRunId),
      enabled: !analysis.isDemo && Boolean(selectedSourceId),
    })),
  })
  const comparisonDetailQueries = useQueries({
    queries: comparisonListingRecords.map((record) => ({
      queryKey: apartmentKeys.listing(record.groupId, comparisonRunId, selectedSourceId),
      queryFn: () => getListing(record.groupId, selectedSourceId as string, comparisonRunId),
      enabled: !analysis.isDemo && Boolean(selectedSourceId),
    })),
  })
  ...
  if (!analysis.isDemo && !selectedSourceId) {
  ...
  if (!analysis.isDemo && (requestedRunMissing || requestedCompareRunMissing)) {
    return <DatasetRequired error="요청한 조사 기록을 찾을 수 없습니다." />
  }
  ...
  const selectResearchDate = (value: string) => {
    ...
    const nextSelectedRunId = apiHistory.find((point) => point.collectedAt === value)?.runId
    const nextComparisonDate = index > 0 ? dates[index - 1] : ''
    const nextCompareRunId = apiHistory.find(
      (point) => point.collectedAt === nextComparisonDate,
    )?.runId
    setSelectedDate(value)
    setComparisonDate(nextComparisonDate)
    const next = new URLSearchParams(searchParams)
    if (nextSelectedRunId) next.set('runId', nextSelectedRunId)
    else next.delete('runId')
    if (nextCompareRunId) next.set('compareRunId', nextCompareRunId)
    else next.delete('compareRunId')
    setSearchParams(next, { replace: true })
  }

  const selectComparisonDate = (value: string) => {
    setComparisonDate(value)
    const next = new URLSearchParams(searchParams)
    const nextCompareRunId = apiHistory.find(
      (point) => point.collectedAt === value,
    )?.runId
    if (nextCompareRunId) next.set('compareRunId', nextCompareRunId)
    else next.delete('compareRunId')
    setSearchParams(next, { replace: true })
        ...
        <div className="rounded-3xl bg-slate-950 p-5 text-white sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:w-[610px]">
              <label className="text-xs font-bold text-slate-300">비교 기준일<select value={effectiveComparisonDate} onChange={(event) => selectComparisonDate(event.target.value)} disabled={!comparisonOptions.length} className="mt-2 h-11 w-full rounded-xl border border-white/15 bg-white/10 px-3 text-sm font-extrabold text-white outline-none disabled:opacity-50">{comparisonOptions.map((date) => <option key={date} value={date} className="text-slate-900">{formatCollectedAt(date)}</option>)}</select></label>
