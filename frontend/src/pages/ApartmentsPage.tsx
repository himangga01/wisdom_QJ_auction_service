  ...
  return (
    <>
            ...
            {apartments.map((apartment) => (
              <tr key={apartment.apartmentId} className="transition hover:bg-slate-50/70">
                <td className="px-5 py-5">
                  <Link onClick={() => onSelect(apartment)} to={`/apartments/${apartment.complexId}`} className="font-extrabold text-slate-950 hover:text-emerald-700">{apartment.complexName}</Link>
                ...
                <td className="px-4 py-5"><Link onClick={() => onSelect(apartment)} to={`/apartments/${apartment.complexId}`} aria-label={`${apartment.complexName} 상세 보기`} className="grid size-9 place-items-center rounded-full bg-slate-100 text-slate-500 hover:bg-emerald-600 hover:text-white"><ArrowRight size={16} /></Link></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid gap-3 md:hidden">
        {apartments.map((apartment) => (
          <Link key={apartment.apartmentId} onClick={() => onSelect(apartment)} to={`/apartments/${apartment.complexId}`} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
